    ...
    private Task LoadTask(string signal, List<string> resourcesPaths, string subfolder)
    {
        return new Task(() =>
        {
            ...
            foreach (string path in resourcesPaths) //lets iterrate our paths
            {
                ...
                loadedOne = GD.Load(fullPath); //try to load

                if (loadedOne == null) //if failed
                {
                    GD.PushWarning("resource [" + fullPath + "] loading failed, skipping.");
                    loadingPercentValue += loadingPercentStep;
            ...
            {
                List<PackedScene> scenesList = loadedResources.OfType<PackedScene>().ToList();

                if (scenesList == null || scenesList.Count == 0)
                {
                    GD.PushWarning("something went wrong. Unable to cast loaded resources. No scenes loaded.");
                ...
                if (resourcesPaths.Count > scenesList.Count)
                {
                    GD.Print(scenesList.Count, " scenes loaded, ", resourcesPaths.Count - scenesList.Count, " skipped.");
                }

                EmitSignal(signal, scenesList);
            }
            else
            {
                if (resourcesPaths.Count > loadedResources.Count)
                {
                    GD.Print(loadedResources.Count, " resources loaded, ", resourcesPaths.Count - loadedResources.Count, " skipped.");
                }
